  ...
  std::vector<std::vector<Index > > & RI)
{
  ...
  return igl::readOBJ(obj_file,V,TC,N,F,FTC,FN,RI);
}
...
IGL_INLINE bool igl::readOBJ(
  ...
  std::vector<std::vector<Index > > & FN,
  std::vector<std::vector<Index > > & RI,
  std::vector<std::vector<Index > > & EE)
{
  // Open file, and check for error
  FILE * obj_file = fopen(obj_file_name.c_str(),"r");
  if(NULL==obj_file)
  ...
  return igl::readOBJ(obj_file,V,TC,N,F,FTC,FN,RI,EE);
}
...
#  define IGL_LINE_MAX 2048
  ...
  while (fgets(line, IGL_LINE_MAX, obj_file) != NULL)
  {
    ...
    if(sscanf(line, "%s",type) == 1)
    {
      ...
      {
        //ignore comments or other shit
      }else if(strlen(type) >= 1&& type[0] == 'c'){
        double r = 0;
        int i=0;
        sscanf(l,"%d %lf\n",&i,&r);
        RI.push_back({i-1,static_cast<int>(r)});
      }else if(strlen(type) >= 1 && type[0] == 'e'){
        int f1 = 0;
        int v1 = 0;
        int f2 = 0;
        int v2 = 0;
        sscanf(l,"%d %d %d %d",&f1,&v1,&f2,&v2);
        EE.push_back({f1,v1,f2,v2});
      }else{
        //ignore any other lines
//        fprintf(stderr,
//                "Warning: readOBJ() ignored non-comment line %d:\n  %s",
  ...
  Eigen::PlainObjectBase<DerivedRI>& RI)
{
  ...
  std::vector<std::vector<int>> vRI;
  bool success = igl::readOBJ(str,vV,vTC,vN,vF,vFTC,vFN,vRI);
  if(!success)
  {
  ...
  if(!vRI.empty() && !vRI[0].empty()){
    bool RI_rect = igl::list_to_matrix(vRI,RI);
    if(!RI_rect){
      printf(format,"RI",igl::min_size(vRI),igl::max_size(vRI));
      return false;
    }
  }
  return true;
  ...
  Eigen::PlainObjectBase<DerivedEE>& EE)
{
  std::vector<std::vector<double> > vV,vTC,vN;
  std::vector<std::vector<int> > vF,vFTC,vFN;
  std::vector<std::vector<int>> vRI,vEE;
  bool success = igl::readOBJ(str,vV,vTC,vN,vF,vFTC,vFN,vRI,vEE);
  if(!success)
  {
    // readOBJ(str,vV,vTC,vN,vF,vFTC,vFN) should have already printed an error
    // message to stderr
  ...
  if(!vEE.empty() && !vEE[0].empty()){
    bool EE_rect = igl::list_to_matrix(vEE,EE);
    if(!EE_rect){
      printf(format,"EE",igl::min_size(vEE),igl::max_size(vEE));
      return false;
    }
  }
  return true;
...
template bool igl::readOBJ<Eigen::Matrix<double, -1, -1, 0, -1, -1>, Eigen::Matrix<double, -1, -1, 0, -1, -1>, Eigen::Matrix<double, -1, -1, 0, -1, -1>, Eigen::Matrix<int, -1, -1, 0, -1, -1>, Eigen::Matrix<int, -1, -1, 0, -1, -1>, Eigen::Matrix<int, -1, -1, 0, -1, -1>, Eigen::Matrix<int, -1, -1, 0, -1, -1> >(std::basic_string<char, std::char_traits<char>, std::allocator<char> >, Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 0, -1, -1> >&, Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 0, -1, -1> >&, Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 0, -1, -1> >&, Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1> >&, Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1> >&, Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1> >&, Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1> >&);
template bool igl::readOBJ<Eigen::Matrix<double, -1, -1, 0, -1, -1>, Eigen::Matrix<double, -1, -1, 0, -1, -1>, Eigen::Matrix<double, -1, -1, 0, -1, -1>, Eigen::Matrix<int, -1, -1, 0, -1, -1>, Eigen::Matrix<int, -1, -1, 0, -1, -1>, Eigen::Matrix<int, -1, -1, 0, -1, -1>, Eigen::Matrix<int, -1, -1, 0, -1, -1>, Eigen::Matrix<int, -1, -1, 0, -1, -1> >(std::__1::basic_string<char, std::__1::char_traits<char>, std::__1::allocator<char> >, Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 0, -1, -1> >&, Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 0, -1, -1> >&, Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 0, -1, -1> >&, Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1> >&, Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1> >&, Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1> >&, Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1> >&, Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1> >&);
#endif
